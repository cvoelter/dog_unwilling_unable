...
demo.data <- read.csv(file = "data/unwilling_unable_demographics.csv") %>%
  select(-comments.dogs, -sound.problem)%>%
...
```{r}
walk.away.data<-agg.data%>%
  filter(Value=="walk_away")%>%
  group_by(subject, condition)%>%
  summarise(mean_away_time=mean(sum_duration))
...
ggplot(data=walk.away.data, aes(x=condition , y=mean_away_time))+
